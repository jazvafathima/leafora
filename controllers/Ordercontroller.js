...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    if (['cancelled','delivered'].includes(order.orderStatus) && order.orderStatus !== orderStatus) {
      req.flash && req.flash('error', `Cannot change status from "${order.orderStatus}".`);
      return res.redirect(`/admin/orders/${id}`);
    }
  ...
  await Product.updateOne(
   ...
   order.items.forEach(item => {
    ...
    req.flash && req.flash('success', `Order status updated to "${orderStatus}".`);
...
item.itemStatus = itemStatus;
    ...
    const allStatuses = order.items.map(i => i.itemStatus || order.orderStatus);
...
exports.approveReturn = async (req, res) => {
  try {
    ...
    // Mark item as approved and returned
...
order.paymentStatus = "refunded";
    ...
    // If no more pending return requests, update order status
    const anyPending = order.items.some(i => i.returnStatus === 'requested');
    if (!anyPending) {
      order.orderStatus = 'returned';
    }
    await order.save();
    req.flash('success', 'Return approved');
...
exports.cancelItem = async (req, res) => {
  try {
    ...
    const allCancelled = order.items.every(i => i.status === 'cancelled' || i.itemStatus === 'cancelled');
    if (allCancelled) {
      order.orderStatus = 'cancelled';
    }
    await order.save();
